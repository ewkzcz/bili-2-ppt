...
def main() -> int:
    ...
    parser.add_argument("--attach-browser-port", type=int,
                        help="复用这个调试端口上已登录的浏览器取网页 AI 字幕，不另开窗口")
    ...
    if need_browser:
        # 网页 AI 字幕脚本自己挑浏览器会话（静默后台、优先复用已登录 B站 的实例），
        # 这里只需要把 BVID 和输出目录交给它。
        cmd = [
            sys.executable,
            str(BROWSER_AI_SCRIPT),
            "--bvid",
            find_bvid(args.source),
            "--out",
            str(work_dir),
        ]
        if args.attach_browser_port:
            cmd.extend(["--attach-port", str(args.attach_browser_port)])
