...
def find_qwen_python(explicit: str | None = None) -> str:
    ...
    if resolved:
        return resolved
    raise RuntimeError(f"ASR Python 不存在：{candidate}")


def qwen_available() -> bool:
    """检查当前或显式指定的运行环境能否加载 Qwen ASR。"""
    if importlib.util.find_spec("qwen_asr"):
        return True
    candidate = os.environ.get("BILI_ASR_PYTHON")
    if not candidate:
        return False
    result = subprocess.run([find_qwen_python(candidate), "-c", "import qwen_asr"], capture_output=True)
