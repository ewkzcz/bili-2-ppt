...
只有一集时由主代理自己纠；多集时主代理先定初版术语表，再按集派子代理并行纠错
（见 [references/subtitle-correction.md](references/subtitle-correction.md) 的「多集并行纠错」）。
这是本技能的交付内容，不是可选项，也不许留给下游：
...
**ASR 一次只跑一个进程**：多集交给同一个进程逐集串行识别，模型只加载一次；不要同时起多个 ASR、
不要调大并发或批大小——本机模型占显存和内存，多路同跑会卡死电脑。ASR 在后台跑时，
调度器会同时做画面采集，这不影响 ASR。
