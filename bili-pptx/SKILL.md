...
1. **不做上游的活**：文字只读字幕阶段的纠错稿和术语表，不再纠错别字、同音字，不统一术语，
   不补采画面，不改知识树分类。上游材料有小缺口就只写确认的部分、把缺口记进 `$WORK/pending.jsonl`，**页面上不写「待补充」，也不把流程打回上游**；
...
4. **自检分两层**：第 1 层 `validate_deck.py` 代码读取校验全量跑、报错全修，不能省；
   ...
   不逐页全量扫描，**最多 3 轮**（见 deck-design 的「自检」）。
...
- **选用**：用哪套模版由调度器在开工前一次问定，记在 `run.config.json` 里，这里直接读、不再问；
  可选模版是现场读 `references/` 下成对的 pptx + html 得到的，不写死；单独使用本技能且没指定时用
  **`Cryo_Academic`**（它不在了就用列表第一个）；
...
**先试样式缓存**。同一套模版的样式表每次都一样，写过一次就按模版名缓存起来：

```bash
"$PY" "$SKILL_ROOT"/bili-pptx/scripts/style_cache.py restore <模版名> --to $WORK/deck/
```

命中（退出码 0）就直接用复制过来的 `deck_style.py`，跳过下面第 1–3 步，只做第 4 步装字体。
模版原件或 `deck_kit.py` 改过时缓存自动失效。没命中就按下面四步读模版，写好样式表后存进缓存：

```bash
"$PY" "$SKILL_ROOT"/bili-pptx/scripts/style_cache.py save <模版名> --from $WORK/deck/
```

建页中改过 `deck_style.py` 的，交付前再存一次，下次直接用改好的版本。
...
**deck 默认交两份**（开工前用户选了只要其中一版，就只出那一版），同一目录，文件名只差一个后缀：
...
1. **定模版、读模版**，写好 `deck_style.py`（见「读模版」，先试样式缓存）；
...
6. **第 2 层看图校验**：只渲染「第 1 层报出的页 ∪ 页清单复查列表 ∪ 每种页型各抽一页」
   ...
   （上游已定稿），不逐页全量扫描。做法见 reading-and-qa。

   **定向看图最多 3 轮**（一轮 = 渲染 → 看图 → 修复）。3 轮后仍剩的版面问题记进待确认清单（`$WORK/pending.jsonl`，类型「PPT 瑕疵」），照常交付，
   不再继续循环；只有用户验收后明确提出继续优化，才再开新的轮次。第 5 步的代码校验不受此限。
