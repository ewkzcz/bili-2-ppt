...
2. **立页清单**：读画面保留清单和文字材料，产出 `pages.plan.json`——每页的页码、功能性短标题、
   要讲清的要点、依据哪些画面、材料里缺什么，以及这一页打算用的版式；
...
6. **渲染自检**：`render_preview.py` 出图 → 逐页找问题（与模版原件并排对照设计语言）→ 改 →
   重渲染受影响的页再看，至少走完一轮。做法见 reading-and-qa。
